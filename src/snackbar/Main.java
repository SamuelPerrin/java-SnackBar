package snackbar;

public class Main{
  public static void main(String[] args) {
    Customer c1 = new Customer("Jane", 45.25);
    Customer c2 = new Customer("Bob", 33.14);

    VendingMachine vm1 = new VendingMachine("Food");
    VendingMachine vm2 = new VendingMachine("Drink");
    VendingMachine vm3 = new VendingMachine("Office");
    VendingMachine[] vms = {vm1, vm2, vm3};

    Snack s1 = new Snack("Chips", 36, 1.75, vm1.getId());
    Snack s2 = new Snack("Chocolate Bar", 36, 1, vm1.getId());
    Snack s3 = new Snack("Pretzel", 30, 2, vm1.getId());
    Snack s4 = new Snack("Soda", 24, 2.50, vm2.getId());
    Snack s5 = new Snack("Water", 20, 2.75, vm2.getId());
    Snack[] snacks = {s1, s2, s3, s4, s5};

    // Processing:
    c1.buySnacks(s4.getTotalCost(3));
    s4.buySnack(3);
    System.out.println("Jane cash on hand $" + c1.getCashOnHand());
    System.out.println("Quantity of Soda is " + s4.getQuantity());

    c1.buySnacks(s3.getTotalCost(1));
    s3.buySnack(1);
    System.out.println("Jane cash on hand $" + c1.getCashOnHand());
    System.out.println("Quantity of Pretzel is " + s3.getQuantity());

    c2.buySnacks(s4.getTotalCost(2));
    s4.buySnack(2);
    System.out.println("Bob cash on hand $" + c2.getCashOnHand());
    System.out.println("Quantity of Soda is " + s4.getQuantity());

    c1.addToCashOnHand(10);
    System.out.println("Jane cash on hand $" + c1.getCashOnHand());

    c1.buySnacks(s2.getTotalCost(1));
    s2.buySnack(1);
    System.out.println("Jane cash on hand $" + c1.getCashOnHand());
    System.out.println("Quantity of Chocolate Bar is " + s2.getQuantity());

    s3.addToQuantity(12);
    System.out.println("Quantity of Pretzel is " + s3.getQuantity());

    c2.buySnacks(s3.getTotalCost(3));
    s3.buySnack(3);
    System.out.println("Bob cash on hand $" + c2.getCashOnHand());
    System.out.println("Quantity of Pretzel is " + s3.getQuantity());

    // STRETCH:
    for (Snack s : snacks) {
      System.out.println("Snack: " + s.getName());
      System.out.println("Vending Machine: " + vms[s.getVendingMachineId() - 1].getName());
      System.out.println("Quantity: " + s.getQuantity());
      System.out.println("Total Cost: $" + s.getTotalCost(s.getQuantity()) + "\n");
    }
  }
}